 ...
 * 5 for laplacian
 */
float ub_w = 0.1; // the upper bound of lambda_dis * beta
int auto_run = 0; // when users do not know the 
...
// Regularizer
void UpdateReg(float *vec_u, float *vec_v, float *vec_error, float sigmoid_score)
{
...
void *TrainLINEThread(void *id)
{
    ...
    while (1)
    {
		...
		if (rns) for (int c = 0; c != dim; c++) vec_error_u[c] -= rho * lam * emb_vertex[c + lu];
...
void PrintParameters()
{
    ...
    printf("RNS: %d\n", rns);
    printf("Regularized NCE: %d, beta: %f\n", reg, beta);
...
int main(int argc, char **argv) 
{
    ...
    if ((i = ArgPos((char *)"-reg", argc, argv)) > 0) reg = atoi(argv[i + 1]);
    if ((i = ArgPos((char *)"-beta", argc, argv)) > 0) beta = (float)atof(argv[i + 1]);

    if ((i = ArgPos((char *)"-gamma", argc, argv)) > 0) gam = (float)atof(argv[i + 1]);
    if ((i = ArgPos((char *)"-ub-w", argc, argv)) > 0) ub_w = (float)atof(argv[i + 1]);
